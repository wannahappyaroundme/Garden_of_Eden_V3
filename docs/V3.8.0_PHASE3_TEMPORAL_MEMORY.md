...
- **Gradual Decay**: S=20.0 parameter for slow forgetting (per user requirement: "망각 곡선이 빠르면 안돼")
...
✅ Ebbinghaus curve: R(t) = max(0.10, e^(-t/20.0))
✅ Retention schedule: 24h→100%, 30d→50%, 90d→10%
...
✅ Auto-prune low retention memories
...
│  ┌──────────────┬──────────────┬────────────────┐          │
│  │ Ebbinghaus   │ Memory       │ Retention      │          │
│  │ Calculator   │ Pinning      │ Stats          │          │
│  └──────────────┴──────────────┴────────────────┘          │
└───┬──────────────┬──────────────┬──────────────────────┬───┘
    │              │              │                      │
┌───▼───┐    ┌─────▼─────┐  ┌────▼────┐        ┌───────▼──────┐
│Decay  │    │  Episodic │  │ Config  │        │   Tauri      │
│Worker │    │  Memory   │  │  Table  │        │   Commands   │
│(24h)  │    │  (SQLite) │  │         │        │   (8 cmds)   │
└───────┘    └───────────┘  └─────────┘        └──────────────┘
```
...
```rust
// Memory management
temporal_pin_memory(memory_id) → Result<(), String>
temporal_unpin_memory(memory_id) → Result<(), String>
temporal_prune_memories(threshold) → Result<usize, String>

// Statistics and monitoring
temporal_get_retention_stats() → Result<RetentionStats, String>
temporal_trigger_decay_update() → Result<usize, String>

// Configuration
temporal_get_config() → Result<DecayConfig, String>
temporal_update_config(config) → Result<(), String>

// Utility
temporal_calculate_retention(days, strength, access, pinned) → Result<f64, String>
```
...
1. **No RAG Integration**: Temporal ranking not yet applied to episodic memory retrieval
   - Phase 4: Modify `rag.rs` to combine semantic + temporal scores

2. **No Persona Evolution**: Learning from high-retention memories not implemented
   - Phase 4: Modify `learning.rs` to extract patterns from successful conversations

3. **Fixed Decay Parameters**: Decay strength is global (20.0 for all memories)
   - Future: Per-memory decay strength based on content type

4. **Simple Access Boost**: Linear 5% per access
   - Future: Logarithmic boost or recency-weighted access counting

### Phase 4 Improvements

1. **RAG Integration**: Combine semantic similarity with retention scores
2. **Persona Evolution**: Learn from high-retention conversation patterns
...
**Next Phase**: 4 (RAG + Persona Integration)

---

## Quick Links

- 📖 [Phase 1 Documentation](./V3.8.0_PHASE1_LAM_INTEGRATION.md)
- 📘 [Phase 2 Documentation](./V3.8.0_PHASE2_STREAMING_VISION.md)
- 📙 [V3.7.0 Release Notes](./V3.7.0_RELEASE_NOTES.md)
